...
# Load combination selection — this should be OUTSIDE the button
load_case = st.selectbox(
    "Select Load Combination (as per IS 456:2000 Clause 36.4.1):",
    [
        "1.5(D + L) — Dead + Live Load",
        "1.35D — Dead Load Only",
        "1.2(D + L + W) — Dead + Live + Wind/Seismic"
    ]
)

# Function to suggest reinforcement
...
if st.button("Calculate Design"):
    ...
    # Apply selected load combination
    if "1.5" in load_case:
        Pu = 1.5 * P
    elif "1.35" in load_case:
        Pu = 1.35 * P
    else:
        Pu = 1.2 * P
    ...
    # Display Results
    st.markdown("### 🔹 Results")
    st.write(f"**Selected Load Combination:** {load_case}")
    ...
    y -= 40
    lines = [
        f"Selected Load Case: {load_case}",
